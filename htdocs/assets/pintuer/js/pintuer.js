$(function() {
	...
	$('.checkall').click(function() {
		var e = $(this);
		var defaultScopt='form';
		var name = e.attr("name");
		var checkfor = e.attr("checkfor");
		var type;
		if(e.attr('scope')&&e.attr('scope')!==''){
			defaultScopt=e.attr('scope');
		}
		if (checkfor != '' && checkfor != null && checkfor != undefined) {
			type = e.closest(defaultScopt).find("input[name='" + checkfor + "']");
		} else {
			type = e.closest(defaultScopt).find("input[type='checkbox']");
		};
		if (name == "checkall") {
...
$showtabs = function(e) {
	...
	e.closest('.tab .tab-nav').find("li").removeClass("active").attr('style','');
	e.closest('.tab').find(".tab-body .tab-panel").removeClass("active");
	e.addClass("active");
	e.attr('style','background-color:#0a8');
	$(detail).addClass("active");
};
var pintuer = {
	/* 如何调用？
	 * pintuer.waterfall({id:".waterfall",width:50,height:50});
	 */
	waterfall: function(val) {
		var tmp_w = "";
		if (pintuer.isjson(val)) {
			var config = {
				id: $(".waterfall"),
				width: "auto",
				height: "auto"
			};
			var fnjson = JSON.parse(JSON.stringify(val));
			if (fnjson.id != undefined) {
				config.id = fnjson.id;
			}
			if (fnjson.width != undefined) {
				config.width = fnjson.width + "px";
				tmp_w = (config.width - 10) + "px";
			}
			if (fnjson.height != undefined) {
				config.height = fnjson.height + "px";
			}
			var e = config.id;
			var w = config.width;
			var h = config.height;
			...
			var e = $(".waterfall");
			if (e.attr("data-width") == null || e.attr("data-width") == "") {
				w = "auto";
			} else {
				w = e.attr("data-width") + "px";
				tmp_w = (e.attr("data-width") - 10) + "px";
			}
			if (e.attr("data-height") == null || e.attr("data-height") == "") {
				h = "auto";
			} else {
				h = e.attr("data-height") + "px";
			}
			$(".waterfall ul").css({
				"-webkit-column-width": "" + w,
				"-moz-column-width": "" + w,
