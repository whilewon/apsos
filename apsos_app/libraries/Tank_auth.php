...
class Tank_auth
{
	...
	{
		return $this->ci->session->userdata('curr_user')->enable === ($activated ? STATUS_ACTIVATED : STATUS_NOT_ACTIVATED);
	}
	...
	{
		return $this->ci->session->userdata('curr_user')->uuid;
	}
	...
	{
		return $this->ci->session->userdata('curr_user')->login_code;
	}
	 ...
	 * @return	bool
	 */
	function is_username_available($login_code)
	{
		return ((strlen($login_code) > 0) AND $this->ci->users->is_username_available($login_code));
	}
